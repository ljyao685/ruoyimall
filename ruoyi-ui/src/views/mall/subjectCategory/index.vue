  ...
  <div :style="{height: mainHeight}">
    <el-container style="height:100%;">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="handleAdd"
              v-hasPermi="['mall:subjectCategory:add']"
            >新增
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="single"
              @click="handleUpdate"
              v-hasPermi="['mall:subjectCategory:edit']"
            >修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['mall:subjectCategory:remove']"
            >删除
            </el-button>
          </el-col>
      ...
      <el-main style="height:100%;padding:0;">
        <el-table ref="table" style="width: 100%;" height="100%" v-loading="loading" :data="subjectCategoryList"
                  @selection-change="handleSelectionChange">
          <el-table-column type="selection" width="55" align="center"/>
          <el-table-column label="专题分类名称" align="center" prop="name"/>
          <el-table-column label="专题分类图标" align="center" prop="icon">
            <template slot-scope="scope">
              <el-image style="width: 30px;height: 30px"
                        :src="scope.row.icon"
                        :preview-src-list="scope.row.icon?scope.row.icon.split(','):[]">
          ...
          <el-table-column label="专题数量" align="center" prop="subjectCount"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body :close-on-click-modal="false">
      <el-form ref="form" v-if="open" :model="form" :rules="rules" label-width="110px">
        <el-form-item label="专题分类名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入专题分类名称"/>
        </el-form-item>
        <el-form-item label="专题分类图标" prop="icon">
...
import {
  addSubjectCategory,
  delSubjectCategory,
  exportSubjectCategory,
  getSubjectCategory,
  listSubjectCategory,
  updateSubjectCategory,
  updateSubjectCategoryStatus
} from "@/api/mall/subjectCategory";
...
export default {
  ...
  methods: {
    ...
    handleStatusChange(row, type) {
      ...
      updateSubjectCategoryStatus(data).then(response => {
    ...
    handleUpdate(row) {
      ...
      getSubjectCategory(id).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除专题分类信息编号为"' + ids + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return delSubjectCategory(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有专题分类信息数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.exportLoading = true;
        return exportSubjectCategory(queryParams);
      }).then(response => {
